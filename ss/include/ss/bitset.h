...
void bitset_throw_range() {
  throw std::out_of_range("ss::bitset::bitset : pos out of range from str");
}
...
void bitset_throw_invalid_argument(const char* msg) {
  throw std::invalid_argument("ss::bitset::bitset : invalid character detected");
}
...
# endif
  ...
  to_string(CharT zero = CharT('0'), CharT one = CharT('1')) const {
    // TODO: Optimize?
    std::basic_string<CharT,Traits,Allocator> result(N, '\0');
    for (size_t i = 0; i < N; ++i) {
...
operator<<(std::basic_ostream<CharT, Traits>& os, const bitset<N>& x) {
  os << x.to_string(std::use_facet<std::ctype<CharT> >(os.getloc()).widen('0'),
