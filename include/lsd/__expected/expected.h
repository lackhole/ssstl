...
#include "lsd/__expected/bad_expected_access.h"
#include "lsd/__expected/monadic.h"
#include "lsd/__expected/unexpected.h"
#include "lsd/__expected/unexpect.h"
...
void reinit_expected_impl(std::false_type, std::false_type,
  ...
  lsd::destroy_at(lsd::addressof(old_val));
  try {
    lsd::construct_at(lsd::addressof(new_val), std::forward<Args>(args)...);
  } catch (...) {
    lsd::construct_at(lsd::addressof(old_val), std::move(temp));
    throw;
  }
}
...
constexpr void reinit_expected_impl(std::false_type, std::true_type,
  ...
  lsd::destroy_at(lsd::addressof(old_val));
  lsd::construct_at(lsd::addressof(new_val), std::move(temp));
}
...
struct expected_storage_t<false, T, E> {
  ...
  LSD_CONSTEXPR_AFTER_CXX20 ~expected_storage_t() noexcept {}
...
struct expected_base {
  ...
  LSD_CONSTEXPR_AFTER_CXX20 void reinit_value_and_destroy_error(Args&&... args) {
  ...
  template<typename U = T, typename Other, std::enable_if_t<!std::is_same<U, void_placeholder_t>::value, int> = 0>
  LSD_CONSTEXPR_AFTER_CXX20 void reinit_value_and_destroy_error_from(Other&& other) {
  ...
  template<typename U = T, typename... Args, std::enable_if_t<!std::is_same<U, void_placeholder_t>::value, int> = 0>
  LSD_CONSTEXPR_AFTER_CXX20 void reinit_error_and_destroy_value(Args&&... args) {
  ...
  constexpr void construct_value(Args&&... args) {
    lsd::construct_at(lsd::addressof(storage.value_), std::forward<Args>(args)...);
  }
  ...
  constexpr void construct_error(G&& e) {
    lsd::construct_at(lsd::addressof(storage.error_), std::forward<G>(e));
  }
  ...
  LSD_CONSTEXPR_AFTER_CXX20 void destroy() {
  ...
  template<typename U = T, std::enable_if_t<!std::is_trivially_destructible<U>::value, int> = 0>
  LSD_CONSTEXPR_AFTER_CXX20 void destroy_value() {
    value().~T();
  }
  ...
  template<typename U = E, std::enable_if_t<!std::is_trivially_destructible<U>::value, int> = 0>
  LSD_CONSTEXPR_AFTER_CXX20 void destroy_error() {
    error().~E();
  }

  LSD_NODISCARD constexpr bool has_value() const noexcept {
...
struct expected_nontrivial_dtor : expected_base<T, E> {
  ...
  LSD_CONSTEXPR_AFTER_CXX20 ~expected_nontrivial_dtor() {
    base::destroy();
  }
...
class expected : private detail::expected_control_smf<detail::void_placdholder_or_t<T>, E> {
 ...
 public:
  ...
  constexpr const T* operator->() const noexcept {
    return lsd::addressof(base::value());
  }
  ...
  LSD_NODISCARD bool has_value() const noexcept {
 ...
 private:
  ...
  static LSD_CONSTEXPR_AFTER_CXX17
  ...
  void swap_value_with_error(expected& thiz, expected& other, std::true_type /* void */, Any) noexcept(error_nothrow_swappable::value) {
    lsd::construct_at(std::addressof(thiz.error()), std::move(other.error()));
    lsd::destroy_at(std::addressof(other.error()));
  }

  static LSD_CONSTEXPR_AFTER_CXX23
  void swap_value_with_error(expected& thiz, expected& other, std::false_type /* void */, std::true_type /* nothrow */) noexcept(nothrow_swappable::value) {
    ...
    lsd::destroy_at(std::addressof(other.error()));
    try {
      lsd::construct_at(std::addressof(other.value()), std::move(thiz.value()));
      lsd::destroy_at(std::addressof(thiz.value()));
      lsd::construct_at(std::addressof(thiz.error()), std::move(temp));
    } catch (...) {
      lsd::construct_at(std::addressof(other.error()), std::move(temp));
      throw;
    }
  }
  ...
  void swap_value_with_error(expected& thiz, expected& other, std::false_type /* void */, std::false_type /* nothrow */) noexcept(nothrow_swappable::value) {
    ...
    try {
      lsd::construct_at(std::addressof(thiz.error()), std::move(other.error()));
      lsd::destroy_at(std::addressof(other.error()));
      lsd::construct_at(std::addressof(other.value()), std::move(temp));
    } catch (...) {
      lsd::construct_at(std::addressof(thiz.value()), std::move(temp));
      throw;
    }
  }
  ...
  constexpr void check_throw() & {
    if (!has_value())
      throw bad_expected_access<std::decay_t<E>>(lsd::as_const(error()));
  }
  constexpr void check_throw() && {
    if (!has_value())
