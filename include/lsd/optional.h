...
#  */
#
# ifndef LSD_OPTIONAL_H_
# define LSD_OPTIONAL_H_
#
# include <exception>
# include <new>
#
...
struct dtor {
  ...
  constexpr const        value_type* pointer() const noexcept { return lsd::addressof(val); }
  LSD_CONSTEXPR_AFTER_14  value_type* pointer()       noexcept { return lsd::addressof(val); }

  constexpr const        value_type& ref() const&  noexcept { return val;       }
  LSD_CONSTEXPR_AFTER_14  value_type& ref()      &  noexcept { return val;       }
  constexpr const        value_type& ref() const&& noexcept { return lsd::move(val); }
  LSD_CONSTEXPR_AFTER_14  value_type& ref()      && noexcept { return lsd::move(val); }

  template<typename ...Args>
  void construct(Args&&... args) {
    ::new((void*)lsd::addressof(val)) value_type(lsd::forward<Args>(args)...);
    valid = true;
  }
  ...
  void construct_if(Other&& other) {
    if (other)
      construct(*lsd::forward<Other>(other));
  }
...
struct dtor<T, false> {
  ...
  constexpr             const value_type* pointer() const { return lsd::addressof(val); }
  LSD_CONSTEXPR_AFTER_14       value_type* pointer()       { return lsd::addressof(val); }

  constexpr             const value_type& ref() const&  { return val;        }
  LSD_CONSTEXPR_AFTER_14       value_type& ref()      &  { return val;        }
  constexpr             const value_type& ref() const&& { return lsd::move(val);  }
  LSD_CONSTEXPR_AFTER_14       value_type& ref()      && { return lsd::move(val);  }
...
struct move_ctor<T, false> : copy_ctor<T> {
  ...
  move_ctor(move_ctor && other) noexcept(is_nothrow_move_constructible<T>::value) {
    if (other.valid) {
      this->construct(lsd::move(other.val));
    }
  }
...
class optional :
 ...
 public:
  ...
  template<typename U,
    ...
    int> = 0>
  LSD_CONSTEXPR_AFTER_14 optional(const optional<U>& other) {
  ...
  template<typename U,
    ...
    int> = 0>
  LSD_CONSTEXPR_AFTER_14 explicit optional(const optional<U>& other) {
    this->construct_if(other);
  }
  ...
  template<typename U,
    enable_if_t<
      ...
      internal::optional::check_convertible  <value_type, optional<U>>::value,
    int> = 0>
  LSD_CONSTEXPR_AFTER_14 optional(optional<U>&& other) {
    this->construct_if(lsd::move(other));
  }
  ...
  template<typename U,
    ...
    int> = 0>
  LSD_CONSTEXPR_AFTER_14 explicit optional(optional<U>&& other) {
    this->construct_if(lsd::move(other));
  }
  ...
  constexpr explicit optional(in_place_t, Arg&& arg, Args&&... args)
    : base(in_place, lsd::forward<Arg>(arg), lsd::forward<Args>(args)...) {}
  ...
  constexpr explicit optional(in_place_t, std::initializer_list<U> ilist, Args&&... args)
    : base(in_place, ilist, lsd::forward<Args>(args)...) {}
  ...
  constexpr optional(U&& value)
    : base(in_place, lsd::forward<U>(value)) {}
  ...
  LSD_CONSTEXPR_AFTER_14 optional& operator=(optional const&) = default;
  LSD_CONSTEXPR_AFTER_14 optional& operator=(optional &&) = default;
  ...
  optional& operator=(U&& value) {
    if (has_value()) {
      this->val = lsd::forward<U>(value);
    } else {
      this->construct(lsd::forward<U>(value));
    }
    return *this;
  }
  ...
  optional& operator=(optional<U>&& other) {
    if (other.has_value()) {
      if (this->has_value())
        this->val = lsd::move(*other);
      else
        this->construct(lsd::move(*other));
    } else { // !other.has_value()
      if (this->has_value())
        this->reset();
    }
  ...
  LSD_CONSTEXPR_AFTER_14       value_type*  operator->()       { return this->pointer();    }

  constexpr             const value_type&  operator*() const&  { return this->ref();       }
  LSD_CONSTEXPR_AFTER_14       value_type&  operator*()      &  { return this->ref();       }
  constexpr             const value_type&& operator*() const&& { return lsd::move(this->ref()); }
  LSD_CONSTEXPR_AFTER_14       value_type&& operator*()      && { return lsd::move(this->ref()); }
  ...
  LSD_CONSTEXPR_AFTER_14 inline value_type& value() & {
    if (!this->has_value())
      throw bad_optional_access{};
    return this->ref();
  }
  LSD_CONSTEXPR_AFTER_14 inline const value_type& value() const& {
    if (!this->has_value())
      throw bad_optional_access{};
    return this->ref();
  }
  ...
  constexpr value_type value_or(U&& default_value) const & {
    static_assert(is_copy_constructible<value_type>::value,
                  "lsd::optional<T>::value_or : T must be copy constructible");
    static_assert(is_convertible<U&&, value_type>::value,
                  "lsd::optional<T>::value_or : U&& must be convertible to T");

    return this->has_value() ? **this : static_cast<value_type>(lsd::forward<U>(default_value));
  }

  template<typename U>
  LSD_CONSTEXPR_AFTER_14 value_type value_or(U&& default_value) && {
    static_assert(is_move_constructible<value_type>::value,
                  "lsd::optional<T>::value_or : T must be move constructible");
    static_assert(is_convertible<U&&, value_type>::value,
                  "lsd::optional<T>::value_or : U&& must be convertible to T");

    return this->has_value() ? lsd::move(**this) : static_cast<value_type>(lsd::forward<U>(default_value));
  }
  ...
  {
    static_assert(is_move_constructible<value_type>::value,
                  "lsd::optional<T>::swap : T must be move constructible");
  ...
  value_type& emplace(Arg&& arg, Args&&... args) {
    this->reset();
    this->construct(lsd::forward<Arg>(arg), lsd::forward<Args>(args)...);
    return **this;
  }
  ...
  value_type& emplace(std::initializer_list<U> ilist, Args&&... args) {
    this->reset();
    this->construct(ilist, lsd::forward<Args>(args)...);
    return **this;
  }
...
constexpr inline optional<decay_t<T>> make_optional(T&& value) {
  return optional<decay_t<T>>(lsd::forward<T>(value));
}

template<typename T, typename ...Args>
constexpr inline optional<T> make_optional(Args&&... args) {
  return optional<T>(in_place, lsd::forward<Args>(args)...);
}

template<typename T, typename U, typename ...Args>
constexpr inline optional<T> make_optional(std::initializer_list<U> ilist, Args&&... args) {
  return optional<T>(in_place, ilist, lsd::forward<Args>(args)...);
}
