...
#include "lsd/__variant/bad_variant_access.h"
#include "lsd/__variant/monostate.h"
#include "lsd/__variant/variant_alternative.h"
...
struct union_wrapper_t<false, T, Types...> {
  ...
  LSD_CONSTEXPR_AFTER_CXX20 ~union_wrapper_t() noexcept {}
...
constexpr decltype(auto) variant_raw_visit_impl2(Union&& u, F&& func, in_place_index_t<0>, Args&&... args) {
  return lsd::invoke(std::forward<F>(func), std::forward<Union>(u), in_place_index<variant_npos>, std::forward<Args>(args)...);
}
...
struct variant_ctor_raw_visitor {
  ...
  LSD_CONSTEXPR_AFTER_CXX17 void operator()(T&& value, in_place_index_t<I>) {
    using U = remove_cvref_t<type_sequence_element_type_t<I, type_sequence<Types...>>>;
    ::new(lsd::addressof(variant_raw_get(base.union_, in_place_index<I>))) U(std::forward<T>(value));
    base.index_ = I;
  }
...
struct variant_assign_raw_visitor {
  ...
  template<typename T, std::size_t I>
  LSD_CONSTEXPR_AFTER_CXX17 void copy_assign_different(T& value, in_place_index_t<I>, std::true_type /* direct construct */) {
    base.reset();
    using U = remove_cvref_t<type_sequence_element_type_t<I, type_sequence<Types...>>>;
    ::new(lsd::addressof(variant_raw_get(base.union_, in_place_index<I>))) U(value);
  }

  template<typename T, std::size_t I>
  LSD_CONSTEXPR_AFTER_CXX17 void copy_assign_different(T& value, in_place_index_t<I>, std::false_type /* direct construct */) {
    auto temp = value;
    base.reset();
    using U = remove_cvref_t<type_sequence_element_type_t<I, type_sequence<Types...>>>;
    ::new(lsd::addressof(variant_raw_get(base.union_, in_place_index<I>))) U(std::move(temp));
  }

  template<typename T, std::size_t I>
  LSD_CONSTEXPR_AFTER_CXX17 void assign_different(T&& value, in_place_index_t<I>) {
    base.reset();
    using U = remove_cvref_t<type_sequence_element_type_t<I, type_sequence<Types...>>>;
    ::new(lsd::addressof(variant_raw_get(base.union_, in_place_index<I>))) U(std::forward<T>(value));
  }
...
template<typename... Types>
struct variant_base {
  LSD_CONSTEXPR_AFTER_CXX20 variant_base()
  ...
  LSD_CONSTEXPR_AFTER_CXX20 void destroy_unchecked() {
  ...
  LSD_CONSTEXPR_AFTER_CXX20 void assign_different(T&& t, in_place_index_t<I>, std::true_type) {
    emplace<I>(std::forward<T>(t));
  }

  template<typename Tj, typename T, std::size_t I>
  LSD_CONSTEXPR_AFTER_CXX20 void assign_different(T&& t, in_place_index_t<I>, std::false_type) {
    emplace<I>(Tj(std::forward<T>(t)));
  }

  template<typename T, std::size_t I>
  LSD_CONSTEXPR_AFTER_CXX20 void assign(T&& t, std::integral_constant<std::size_t, I>) {
    if (index_ == I) {
  ...
  LSD_CONSTEXPR_AFTER_CXX20 type_sequence_element_type_t<I, type_sequence<Types...>>&
  emplace(Args&&... args) {
    reset();
  ...
  emplace_valueless(Args&&... args) {
    ...
    ::new(lsd::addressof(elem)) U(std::forward<Args>(args)...);
    index_ = I;
    return elem;
  }
...
struct variant_nontrivial_dtor : variant_base<Types...> {
  ...
  LSD_CONSTEXPR_AFTER_CXX20 ~variant_nontrivial_dtor() {
...
class variant : private detail::variant_control_smf<Types...> {
 ...
 public:
  ...
  template<typename T, typename... Args, std::enable_if_t<conjunction<
      ...
      bool_constant<( TypeSeq::template unique<T> )>
  >::value, int> = 0>
  LSD_CONSTEXPR_AFTER_CXX20 T& emplace(Args&&... args) {
  ...
  template<typename T, typename U, typename... Args, std::enable_if_t<conjunction<
      ...
      bool_constant<( TypeSeq::template unique<T> )>
  >::value, int> = 0>
  LSD_CONSTEXPR_AFTER_CXX20 T& emplace(std::initializer_list<U> il, Args&&... args) {
  ...
  ::value, int> = 0>
  LSD_CONSTEXPR_AFTER_CXX20 variant_alternative_t<I, variant>& emplace(Args&&... args) {
  ...
  ::value, int> = 0>
  LSD_CONSTEXPR_AFTER_CXX20 variant_alternative_t<I, variant>& emplace(std::initializer_list<T> il, Args&&... args) {
 ...
 private:
  struct visitor_self {
    ...
    constexpr decltype(auto) operator()(T&& x, in_place_index_t<I>, Visitor&& vis) const {
      return lsd::invoke(std::forward<Visitor>(vis), std::forward<T>(x));
    }
    ...
    constexpr decltype(auto) operator()(Union&& u, in_place_index_t<variant_npos>, Visitor&& vis) const {
      ...
      return lsd::invoke(std::forward<Visitor>(vis), std::forward<Union>(u).get());
    }
  };
};
...
get_if(variant<Types...>* pv) noexcept {
  if (pv->index() != I)
    return nullptr;
  return lsd::addressof(detail::variant_raw_get(pv->_base().storage(), in_place_index<I>));
}
