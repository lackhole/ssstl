...
class basic_string_view {
 public:
  ...
  constexpr basic_string_view(It first, End last)
      : data_(lsd::to_address(first)), size_(last - first) {}
  ...
  constexpr const_reference at(size_type pos) const {
    if (pos >= size()) {
      throw std::out_of_range("lsd::string_view::at : out of range");
    }
    return (*this)[pos];
  }
  ...
  constexpr size_type copy(value_type* dest, size_type count, size_type pos = 0) const {
    if (pos > size()) {
      throw std::out_of_range("lsd::string_view::copy : out of range");
    }
  ...
  constexpr basic_string_view substr(size_type pos = 0, size_type count = npos) const {
    if (pos > size()) {
      throw std::out_of_range("lsd::string_view::substr : out of range");
    }
  ...
  constexpr size_type find(value_type c, size_type pos = 0) const noexcept {
    return find(basic_string_view(lsd::addressof(c), 1), pos);
  }
  ...
  constexpr size_type rfind(value_type c, size_type pos = npos) const noexcept {
    return rfind(basic_string_view(lsd::addressof(c), 1), pos);
  }
  ...
  constexpr size_type find_first_of(value_type c, size_type pos = 0) const noexcept {
    return find_first_of(basic_string_view(lsd::addressof(c), 1), pos);
  }
  ...
  constexpr size_type find_last_of(value_type c, size_type pos = npos) const noexcept {
    return find_last_of(basic_string_view(lsd::addressof(c), 1), pos);
  }
  ...
  constexpr size_type find_first_not_of(value_type c, size_type pos = 0) const noexcept {
    return find_first_not_of(basic_string_view(lsd::addressof(c), 1), pos);
  }
  ...
  constexpr size_type find_last_not_of(value_type c, size_type pos = npos) const noexcept {
    return find_last_not_of(basic_string_view(lsd::addressof(c), 1), pos);
  }
...
template<>
struct hash<lsd::string_view> {
  std::size_t operator()(const lsd::string_view& sv) const noexcept {
    return lsd::hash_array(sv.data(), sv.size());
  }
};

template<>
struct hash<lsd::wstring_view> {
  std::size_t operator()(const lsd::wstring_view& sv) const noexcept {
    return lsd::hash_array(sv.data(), sv.size());
  }
};
...
#if __cplusplus >= 202002L
template<>
struct hash<lsd::u8string_view> {
  std::size_t operator()(const lsd::u8string_view& sv) const noexcept {
    return lsd::hash_array(sv.data(), sv.size());
  }
};
#endif
...
template<>
struct hash<lsd::u16string_view> {
  std::size_t operator()(const lsd::u16string_view& sv) const noexcept {
    return lsd::hash_array(sv.data(), sv.size());
  }
};

template<>
struct hash<lsd::u32string_view> {
  std::size_t operator()(const lsd::u32string_view& sv) const noexcept {
    return lsd::hash_array(sv.data(), sv.size());
  }
};
