...
#ifndef LSD_RANGES_VIEWS_FILTER_VIEW_HPP
#define LSD_RANGES_VIEWS_FILTER_VIEW_HPP
  ...
  class iterator : public detail::filter_view_iterator_category<V> {
   public:
    ...
    constexpr iterator(filter_view& parent, iterator_t<V> current)
        : current_(std::move(current)), parent_(lsd::addressof(parent)) {}
    ...
    constexpr iterator& operator--() {
      do {
        --current_;
      } while (!lsd::invoke(*parent_->pred_, *current_));
      return *this;
    }
...
#endif // LSD_RANGES_VIEWS_FILTER_VIEW_HPP
