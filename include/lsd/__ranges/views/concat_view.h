...
struct concat_view : view_interface<concat_view<Rngs...>> {
 ...
 public:
  ...
  struct iterator {
   ...
   private:
    ...
    struct prev_raw_visitor {
      ...
      void operator()(I& it, in_place_index_t<N>) {
        if (it == ranges::begin(std::get<N>(pos->parent_->bases_))) {
          ...
          lsd::detail::variant_raw_visit(pos->its_.index(), pos->its_._base().storage(), *this);
        } else {
          --it;
        }
      }
    ...
    static difference_type distance_to_(std::integral_constant<size_t, N>, const iterator& from, const iterator& to) {
      ...
      if (from.its_.index() == N) {
        if (to.its_.index() == N)
          return ranges::distance(lsd::detail::variant_raw_get(from.its_._base().storage(), in_place_index<N>),
                                  lsd::detail::variant_raw_get(to.its_._base().storage(), in_place_index<N>));
        return ranges::distance(lsd::detail::variant_raw_get(from.its_._base().storage(), in_place_index<N>),
   ...
   public:
    ...
    constexpr iterator& operator++() {
      lsd::detail::variant_raw_visit(its_.index(), its_._base().storage(), next_raw_visitor{this});
      return *this;
    }
    ...
    >::value, int> = 0>
    constexpr iterator& operator--() {
      lsd::detail::variant_raw_visit(its_.index(), its_._base().storage(), prev_raw_visitor{this});
      return *this;
    }
    ...
    constexpr iterator& operator+=(difference_type n) {
      if (n > 0) {
        lsd::detail::variant_raw_visit(its_.index(), its_._base().storage(), advance_fwd_raw_visitor{this, n});
      } else if (n < 0) {
        lsd::detail::variant_raw_visit(its_.index(), its_._base().storage(), advance_rev_raw_visitor{this, n});
      }
      return *this;
    }
  ...
  >::value, int> = 0>
  constexpr auto size() {
    return lsd::tuple_fold_left(
        lsd::tuple_transform(bases_, [](auto&& r) { return r.size(); }),
        std::size_t{0},
        std::plus<>{}
    );
  }
...
constexpr LSD_INLINE_OR_STATIC detail::concat_niebloid concat{};
