...
#ifndef LSD_RANGES_VIEWS_REPEAT_VIEW_HPP
#define LSD_RANGES_VIEWS_REPEAT_VIEW_HPP
...
class repeat_view : public view_interface<repeat_view<W, Bound>> {
 public:
  ...
  LSD_CONSTEXPR_AFTER_CXX17 iterator begin() const {
    return iterator(lsd::addressof(*value_));
  }

  template<typename B = Bound, std::enable_if_t<
      negation< same_as<B, unreachable_sentinel_t> >
  ::value, int> = 0>
  LSD_CONSTEXPR_AFTER_CXX17 iterator end() const {
    return iterator(lsd::addressof(*value_), bound_);
  }
...
#endif // LSD_RANGES_VIEWS_REPEAT_VIEW_HPP
