...
#ifndef LSD_RANGES_VIEWS_JOIN_WITH_VIEW_HPP_
#define LSD_RANGES_VIEWS_JOIN_WITH_VIEW_HPP_
          ...
          maybe_const<Const, Pattern>>
    {
     ...
     public:
        ...
        switch (i.inner_it_.index()) {
          case 0:
            inner_it_.template emplace<0>(std::move(
                lsd::detail::variant_raw_get(i.inner_it_._base().union_, in_place_index<0>)));
            break;
          case 1:
            inner_it_.template emplace<1>(std::move(
                lsd::detail::variant_raw_get(i.inner_it_._base().union_, in_place_index<1>)));
            break;
          default:
      ...
      constexpr iterator& operator--() {
        ...
        for (;;) {
          if (inner_it_.index() == 0) {
            auto& it = lsd::detail::variant_raw_get(inner_it_._base().union_, in_place_index<0>);
            ...
            auto& it = lsd::detail::variant_raw_get(inner_it_._base().union_, in_place_index<1>);
      ...
      friend constexpr void iter_swap(const iterator& x, const iterator& y) {
        lsd::visit(ranges::iter_swap, x.inner_it_, y.inner_it_);
      }
     ...
     private:
      ...
      constexpr iterator(Parent& parent, iterator_t<Base> outer_it)
          : iterator_base(std::move(outer_it)), parent_(lsd::addressof(parent))
      {
        using namespace lsd::rel_ops;
      ...
      constexpr void satisfy() {
        ...
        for (;;) {
          if (inner_it_.index() == 0) {
            if (lsd::detail::variant_raw_get(inner_it_._base().union_, in_place_index<0>) != ranges::end(parent_->pattern_)) {
              break;
            }

            inner_it_.template emplace<1>(ranges::begin(update_inner()));
          } else {
            if (lsd::detail::variant_raw_get(inner_it_._base().union_, in_place_index<1>) != ranges::end(get_inner())) {
              break;
            }
...
#endif // LSD_RANGES_VIEWS_JOIN_WITH_VIEW_HPP_
