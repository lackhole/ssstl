...
#include "lsd/__compare/three_way_comparable.h"
        ...
        type_sequence<Views...>>
  {
    ...
    struct deref_fn {
     private:
      struct impl {
        ...
        decltype(auto) operator()(const F& f, const Tuple& t, std::index_sequence<I...>)
            noexcept(noexcept(lsd::invoke(f, std::get<I>(t)...)))
        {
          return lsd::invoke(f, std::get<I>(t)...);
        }
      };
     ...
     public:
      ...
      decltype(auto) operator()(const F& f, const Tuple& t)
          noexcept(noexcept(lsd::invoke(impl{}, f, t, tuple_index_sequence{})))
      {
   ...
   public:
    ...
    constexpr decltype(auto) operator*() const
        noexcept(noexcept(lsd::invoke(deref_fn{}, *parent_->fun_, *inner_)))
    {
    ...
    constexpr decltype(auto) operator[](difference_type n) const {
      return lsd::apply(
          [&](const auto&... iters) -> decltype(auto) {
            return lsd::invoke(*parent_->fun_, iters[iter_difference_t<remove_cvref_t<decltype(iters)>>(n)]...);
          }
      );
    }
   ...
   private:
    LSD_CONSTEXPR_AFTER_CXX17 iterator(Parent& parent, ziperator<Const> inner)
