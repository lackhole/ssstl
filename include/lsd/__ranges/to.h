...
using lsd::detail::tag_4;
using lsd::detail::tag_5;
...
to(R&& r, Args&&... args) {
  ...
  return lsd::ranges::to<DEDUCE_EXPR>(std::forward<R>(r), std::forward<Args>(args)...);
}
...
template<typename C, typename R, typename... Args>
constexpr C to_1(tag_5, R&& r, Args&&... args) {
  return lsd::ranges::to<C>(r | views::transform([](auto&& elem) {
...
class to_adaptor_closure : public range_adaptor_closure<to_adaptor_closure<C, Args...>> {
 ...
 private:
  ...
  constexpr auto call(R&& r, std::index_sequence<I...>) const& {
    return lsd::ranges::to<C>(std::forward<R>(r), std::get<I>(tup_)...);
  }
  template<typename R, std::size_t... I>
  constexpr auto call(R&& r, std::index_sequence<I...>) && {
    return lsd::ranges::to<C>(std::forward<R>(r), std::get<I>(std::move(tup_))...);
  }
