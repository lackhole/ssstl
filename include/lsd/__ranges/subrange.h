    ...
    , detail::subrange_size<I, S, (K == subrange_kind::sized && sized_sentinel_for<S, I>::value == false)>
{
 ...
 public:
  ...
  LSD_NODISCARD constexpr I begin() {
  ...
  LSD_NODISCARD constexpr subrange_kind prev(iter_difference_t<I> n = 1) const {
    auto tmp = *this;
    tmp.advance(-n);
    return tmp;
  }
  ...
  LSD_NODISCARD constexpr subrange_kind next(iter_difference_t<I> n = 1) const& {
    auto tmp = *this;
    tmp.advance(n);
    return tmp;
  }
  ...
  template<typename I2 = I, std::enable_if_t<forward_iterator<I2>::value, int> = 0>
  LSD_NODISCARD constexpr subrange_kind next(iter_difference_t<I> n = 1) && {
    advance(n);
...
template<typename I, typename S, lsd::ranges::subrange_kind K>
struct std::tuple_size<lsd::ranges::subrange<I, S, K>> : std::integral_constant<std::size_t, 2> {};

namespace std {

using ::lsd::ranges::get;

template<typename I, typename S, lsd::ranges::subrange_kind K>
struct tuple_element<0, lsd::ranges::subrange<I, S, K>> { using type = I; };
template<typename I, typename S, lsd::ranges::subrange_kind K>
struct tuple_element<0, const lsd::ranges::subrange<I, S, K>> { using type = I; };
template<typename I, typename S, lsd::ranges::subrange_kind K>
struct tuple_element<1, lsd::ranges::subrange<I, S, K>> { using type = S; };
template<typename I, typename S, lsd::ranges::subrange_kind K>
struct tuple_element<1, const lsd::ranges::subrange<I, S, K>> { using type = S; };
