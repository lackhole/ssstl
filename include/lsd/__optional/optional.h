...
#  */
#
# ifndef LSD_OPTIONAL_OPTIONAL_H_
# define LSD_OPTIONAL_OPTIONAL_H_
#
# include <type_traits>
# include <new>
# include <utility>
...
# include "lsd/__optional/internal/check_overload.h"
# include "lsd/__optional/internal/move_assignment.h"
# include "lsd/__optional/internal/traits.h"
# include "lsd/__optional/bad_optional_access.h"
# include "lsd/__optional/nullopt_t.h"
# include "lsd/__optional/swap.h"
...
class optional :
 ...
 public:
  ...
  constexpr auto and_then(F&& f) & {
    if (*this)
      return lsd::invoke(std::forward<F>(f), **this);
  ...
  constexpr auto and_then(F&& f) && {
    if (*this)
      return lsd::invoke(std::forward<F>(f), std::move(**this));
    return remove_cvref_t<invoke_result_t<F, value_type>>{};
  }
  ...
  constexpr auto transform(F&& f) & {
    ...
    if (*this)
      return optional<U>(lsd::invoke(std::forward<F>(f), **this));
    return optional<U>{};
  }
  ...
  constexpr auto transform(F&& f) && {
    using U = std::remove_cv_t<invoke_result_t<F, T>>;
    if (*this)
      return optional<U>(lsd::invoke(std::forward<F>(f), std::move(**this)));
    return optional<U>{};
  }
  ...
  noexcept(std::is_nothrow_move_constructible<value_type>::value && lsd::is_nothrow_swappable<value_type>::value)
  {
    static_assert(std::is_move_constructible<value_type>::value,
...
# endif // LSD_OPTIONAL_OPTIONAL_H_
