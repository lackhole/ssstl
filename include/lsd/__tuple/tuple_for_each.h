...
constexpr void tuple_for_each_impl(std::index_sequence<I...>, Tuple&& t, F&& f) {
  int dummy[] = {
      (lsd::invoke(f, std::get<I>(std::forward<Tuple>(t))), 0)...
  };
  (void)dummy;
}
...
constexpr void tuple_for_each_index_impl(std::index_sequence<I...>, Tuple&& t, F&& f) {
  int dummy[] = {
      (lsd::invoke(f, std::get<I>(std::forward<Tuple>(t)), std::integral_constant<std::size_t, I>{}), 0)...
  };
  (void)dummy;
}
...
tuple_for_each(Tuple&& t, F&& f) {
  return lsd::detail::tuple_for_each_impl(
...
tuple_for_each_index(Tuple&& t, F&& f) {
  return lsd::detail::tuple_for_each_index_impl(
      detail::tuple_index_sequence<Tuple>{},
      std::forward<Tuple>(t),
      std::forward<F>(f)
  );
